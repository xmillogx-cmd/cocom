"""
COCOM v11: ListOps Training with Grouped Tokenization

Uses tokens_conv1d for grouped token embedding:
- 2048 tokens → Conv1d with kernel=N → 2048/N positions
- Reduces sequence length, captures local context
"""
...
class ListOpsTrainerV11_Grouped:
    """Trainer for COCOM v11 with grouped tokenization."""
    
    STAGES = [
        {'name': 'full', 'epochs': 30, 'trainable': ['all'], 'lr': 3e-4, 'warmup': True},
    ]
    ...
    def __init__(
        ...
        device: str = 'cuda',
        use_fp16: bool = True,
        group_size: int = 8,  # 8-symbol grouping
    ):
        ...
        # Load data
        self.train_data = ListOpsDataset(data_dir, 'train')
        self.test_data = ListOpsDataset(data_dir, 'test')
        ...
        print(f"Grouped: kernel_size={group_size}, new_seq_len={seq_len // group_size}")
        ...
        self.model = COCOMv11(
            ...
            num_classes=10,
            seq_len=seq_len,
            input_type='tokens_conv1d',  # ← Grouped tokens!
            vocab_size=vocab_size,
            embed_kernel_size=group_size,  # N-symbol grouping
            use_rope=True,
            rope_dims=1,
        ).to(self.device)
        
        params = sum(p.numel() for p in self.model.parameters())
        print(f"\nCOCOMv11 (tokens_conv1d) Parameters: {params:,}")
        print(f"Sequence length: {seq_len} → {self.model.seq_len}")
    ...
    def run(self):
        print("\n" + "="*70)
        print("COCOM v11: LISTOPS with GROUPED TOKENIZATION")
        print("="*70)
        
        print(f"\nKey: tokens_conv1d with kernel={self.group_size}")
        print(f"Sequence: 2048 → {self.model.seq_len} positions")
            ...
            for epoch in range(epochs):
                ...
                if test_metrics['acc'] > stage_best:
                    ...
                    torch.save(self.model.state_dict(), 
                              self.checkpoint_dir / f"cocom_v11_listops_grouped_{stage_name}.pt")
                    saved = " 💾"
                    
                    if test_metrics['acc'] > best_acc:
                        best_acc = test_metrics['acc']
                        torch.save(self.model.state_dict(), 
                                  self.checkpoint_dir / "cocom_v11_listops_grouped_best.pt")
                else:
                    no_improve += 1
        ...
        torch.save(self.model.state_dict(), 
                  self.checkpoint_dir / "cocom_v11_listops_grouped_full.pt")
        
        with open(self.checkpoint_dir / "cocom_v11_listops_grouped_history.json", 'w') as f:
...
if __name__ == "__main__":
    ...
    trainer = ListOpsTrainerV11_Grouped(
